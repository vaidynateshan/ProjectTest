...
def test_saving_the_same_message_twice_is_a_no_op(store: ConversationStore) -> None:
    message = _message()
    assert store.save_message(message) is True
    assert store.save_message(message) is False
    assert len(store.read_thread("15550001111")) == 1


def test_thread_returns_messages_oldest_first(store: ConversationStore) -> None:
    store.save_message(_message(id="wamid.2", timestamp=NOW, text="second"))
    store.save_message(_message(id="wamid.1", timestamp=NOW - 60, text="first"))
...
def test_window_is_open_after_a_recent_inbound_message(store: ConversationStore) -> None:
    store.save_message(_message(timestamp=NOW - 60))
...
def test_window_is_closed_after_25_hours(store: ConversationStore) -> None:
    store.save_message(_message(timestamp=NOW - CUSTOMER_SERVICE_WINDOW_SECONDS - 3600))
...
def test_threads_are_ordered_by_most_recent_activity(store: ConversationStore) -> None:
    store.save_message(_message(id="a", wa_id="111", timestamp=NOW - 100))
    store.save_message(_message(id="b", wa_id="222", timestamp=NOW))
...
def test_contact_name_is_not_overwritten_by_a_later_blank(
    ...
    store.save_message(_message(id="wamid.1", profile_name="Ada"))
    store.save_message(_message(id="wamid.2", profile_name=None, timestamp=NOW + 10))
...
def test_search_finds_text_and_escapes_wildcards(store: ConversationStore) -> None:
    store.save_message(_message(id="wamid.1", text="refund please"))
    store.save_message(_message(id="wamid.2", text="100% happy"))
...
def test_messages_in_the_same_second_keep_insertion_order(
    ...
    store.save_message(_message(id="wamid.1", timestamp=NOW, text="first"))
    store.save_message(_message(id="wamid.2", timestamp=NOW, text="second"))
    store.save_message(_message(id="wamid.3", timestamp=NOW, text="third"))
